		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glPushMatrix(); //llanta frontal der
					...
					glRotatef(-rotTires,1,0, giroruedas);
					glRotatef(-rotTiresY, 1, 0, 0);
					llanta.GLrender(NULL,_SHADED,1.0);
				glPopMatrix();
...
void animacion()
{
	...
	if (circuito)
	{
		...
		if (recorrido6)   //recorrido3
		{
			movKitY += 0.3;
			movKitZ += 0.6;
			rotTiresY++;

			if (movKitZ > -30 && movKitY > 60)
			{
				recorrido6 = false;
				recorrido7 = true;
			}
		}
		if (recorrido7) //recorrido4
		{
			rotTiresY++;
			movKitZ++;
		...
		if (recorrido8) //recorrido 5
		{
			rotTiresY++;
			movKitY -= 0.3;
			movKitZ += 0.6;

			if (movKitZ > 155 && movKitY < 0)
			{
				recorrido8 = false;
				recorrido9 = true;
			}
