...
float posX = 0, posY = 0, posZ = -3.5, rotRodIzq = 0;
float giroMonito = 0;
float movbrazo = 0.0f;
float movbrazoderecho = 0.0f;
float movpiernader = 0.0f;
...
#define MAX_FRAMES 4  ///cantidad de frames
int i_max_steps = 700;
int i_curr_steps = 0;
...
typedef struct _frame
{
	...
	float giroMonito;
	float giroMonitoInc;
	float movbrazo;  //varible   siempre hya que tener la variable mas su incremento
	float movbrazoInc; // variable para hacer los calculos
	float movbrazoderecho;
	float movbrazoderechoInc;
	float movpiernader;
	float movpiernaderInc;
	float movindi1;
	float movindi1Inc;
	float movindi2;
	float movindi2Inc;
...
void saveFrame ( void )
{
	...
	KeyFrame[FrameIndex].posX=posX;   //aw gurda el cuadro clave
	KeyFrame[FrameIndex].posY=posY;
	KeyFrame[FrameIndex].posZ=posZ;

	KeyFrame[FrameIndex].rotRodIzq=rotRodIzq;
	KeyFrame[FrameIndex].giroMonito=giroMonito;
	KeyFrame[FrameIndex].movbrazo = movbrazo; //guarda las frame del movimineto de brazo
	KeyFrame[FrameIndex].movbrazoderecho = movbrazoderecho;
	KeyFrame[FrameIndex].movpiernader = movpiernader;
	
	KeyFrame[FrameIndex].movindi1 = movindi1;
	KeyFrame[FrameIndex].movindi2 = movindi2;
	
	KeyFrame[FrameIndex].movpulgar1 = movpulgar1;
	KeyFrame[FrameIndex].movpulgar2 = movpulgar2;

	KeyFrame[FrameIndex].movmedio1 = movmedio1;
	KeyFrame[FrameIndex].movmedio2 = movmedio2;

	KeyFrame[FrameIndex].movmeñi1 = movmeñi1;
	KeyFrame[FrameIndex].movmeñi2 = movmeñi2;

	KeyFrame[FrameIndex].tijera = tijera;
	KeyFrame[FrameIndex].tijera2 = tijera2;

	KeyFrame[FrameIndex].señal = señal;
...
{
	posX=KeyFrame[0].posX; // se borra lo que se esta dibujando y cuando se reproduce la animacio se pone el framme 0
	...
	movbrazoderecho = KeyFrame[0].movbrazoderecho;
	movpiernader = KeyFrame[0].movpiernader;
	
	movindi1 = KeyFrame[0].movindi1;
	movindi2 = KeyFrame[0].movindi2;

	movpulgar1 = KeyFrame[0].movpulgar1;
	movpulgar2 = KeyFrame[0].movpulgar2;
	
	movmedio1 = KeyFrame[0].movmedio1;
	movmedio2 = KeyFrame[0].movmedio2;

	movmeñi1 = KeyFrame[0].movmeñi1;
	movmeñi2 = KeyFrame[0].movmeñi2;

	tijera = KeyFrame[0].tijera;
	tijera2 = KeyFrame[0].tijera2;

	señal = KeyFrame[0].señal;

}

void interpolation ( void )
{
	KeyFrame[playIndex].incX = (KeyFrame[playIndex + 1].posX - KeyFrame[playIndex].posX) / i_max_steps;	  //vf-vi /numero d e res
	...
	KeyFrame[playIndex].movbrazoderechoInc = (KeyFrame[playIndex + 1].movbrazoderecho - KeyFrame[playIndex].movbrazoderecho) / i_max_steps;
	KeyFrame[playIndex].movpiernaderInc = (KeyFrame[playIndex + 1].movpiernader - KeyFrame[playIndex].movpiernader) / i_max_steps;
	
	KeyFrame[playIndex].movindi1Inc = (KeyFrame[playIndex + 1].movindi1 - KeyFrame[playIndex].movindi1) / i_max_steps;
	KeyFrame[playIndex].movindi2Inc = (KeyFrame[playIndex + 1].movindi2 - KeyFrame[playIndex].movindi2) / i_max_steps;

	KeyFrame[playIndex].movpulgar1Inc = (KeyFrame[playIndex + 1].movpulgar1 - KeyFrame[playIndex].movpulgar1) / i_max_steps;
	KeyFrame[playIndex].movpulgar2Inc = (KeyFrame[playIndex + 1].movpulgar2 - KeyFrame[playIndex].movpulgar2) / i_max_steps;

	KeyFrame[playIndex].movmedio1Inc = (KeyFrame[playIndex + 1].movmedio1 - KeyFrame[playIndex].movmedio1) / i_max_steps;
	KeyFrame[playIndex].movmedio2Inc = (KeyFrame[playIndex + 1].movmedio2 - KeyFrame[playIndex].movmedio2) / i_max_steps;

	KeyFrame[playIndex].movmeñi1Inc = (KeyFrame[playIndex + 1].movmeñi1 - KeyFrame[playIndex].movmeñi1) / i_max_steps;
	KeyFrame[playIndex].movmeñi2Inc = (KeyFrame[playIndex + 1].movmeñi2 - KeyFrame[playIndex].movmeñi2) / i_max_steps;

	KeyFrame[playIndex].tijeraInc = (KeyFrame[playIndex + 1].tijera - KeyFrame[playIndex].tijera) / i_max_steps;
	KeyFrame[playIndex].tijera2Inc = (KeyFrame[playIndex + 1].tijera2 - KeyFrame[playIndex].tijera2) / i_max_steps;

	KeyFrame[playIndex].señalInc = (KeyFrame[playIndex + 1].señal - KeyFrame[playIndex].señal) / i_max_steps;
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix(); //Brazo derecho-->
			...
			glPushMatrix();
				...
				glRotatef(-45, 0, 1, 0); 
				glRotatef(movbrazo, 0.0, 0.0, 1.0);   // rotacion sobre el eje Z
		...
		glPushMatrix(); //Brazo izquierdo <--
			...
			glPushMatrix();
				...
				glRotatef(25, 0, 0, 1);
				glRotatef(25, 1, 0, 0);
				glRotatef(movbrazoderecho, 0, 0, 1);
				glTranslatef(-0.75, 0, 0);
				fig7.prisma(0.7, 1.5, 0.7, 0);
			glPopMatrix();
		glPopMatrix();
		...
		glPushMatrix();//Cintura
			...
			glPushMatrix(); //Pie Derecho -->
				...
				glPushMatrix();
					glTranslatef(0, -0.5, 0);
					glRotatef(15+ movpiernader , 1, 0, 0);
...
void mano(void) {
	...
	glRotatef(movhombro, 0.0, 0.0, 1.0);   // Movienmiento del hombro sobre el eje Z de 0° a 90° y de 0° a -110°
	glRotatef(90, 0.0, 0.0, 1.0);									   // Cuenta desde el origen
	glRotatef(-90, 1.0, 0.0, 0.0);
	glTranslatef(1.25, 0.0, 0.0);          // Centro del prisma A
	...
	glPushMatrix();// Cuenta desde el centro del prisma A
		...
		prisma(text2.GLindex); // Prisma A
		
	glPopMatrix();

	// Brazo
	glTranslatef(1.25, 0.0, 0.0);          // Punto de rotación del codo entre A y B 
	glRotatef(movcodo, 0.0, 1.0, 0.0);     // Rotacion del codo sobre eje Y  0° a 90° y de 90° a 0°
	glTranslatef(1.0, 0.0, 0.0);           // Centro del prisma B
		glPushMatrix();
			glScalef(2.0, 1.0, 1.0);
			glColor3f(0.1, 0.0, 0.5);
			prisma(text2.GLindex);                          // Prisma B
		glPopMatrix();

	// Dedo Indice
		glPushMatrix();                           // Regresamos al centro del prisma B
			glTranslatef(1.0, 0.35, 0.375);       // Punto de rotación del dedo indice1 entre B y C
			glRotatef(tijera, 0.0, 1.0, 0.0);     //Giro de 10 grados sobre Y
			glRotatef(movindi1, 0.0, 0.0, 1.0);   // Rotacion del pulgar de sobre eje Z  50° > 0° < -90°
			glTranslated(0.375, 0.0, 0.0);        // Centro de Prisma C
				glPushMatrix();
					glScalef(0.75, 0.3, 0.25);
					glColor3f(0.1, 0.5, 0.1);
					prisma(text2.GLindex);                           //  Prisma C
				glPopMatrix();
			glTranslatef(0.375, 0.0, 0.0);         //Punto de rotación del dedo medio2 entre C y D
			glRotatef(movindi2, 0.0, 0.0, 1.0);    // Rotacion del dedo indice2 sobre eje Z  50° > 0° < -90°
			glTranslated(0.5, 0.0, 0.0);           // Centro de Prisma D
			glScalef(1.0, 0.3, 0.25);
			glColor3f(0.1, 0.5, 0.1);
			prisma(text2.GLindex);                            // Prisma D
		glPopMatrix();

	// Dedo Pulgar
		glPushMatrix();                            // Regresamos al centro del prisma B
			glTranslatef(1.0, -0.35, 0.375);       // Punto de rotación del dedo pulgar entre B y E
			glRotatef(movpulgar1, 0.0, 1.0, 0.0);  // Rotacion del pulgar de sobre eje Z de 0° a 90°
			glRotatef(señal, 0.0, 1.0, 0.0);
			glTranslated(0.375, 0.0, 0.0);         // Centro de Prisma E
			glPushMatrix();
				glScalef(0.75, 0.3, 0.25);
				glColor3f(0.2, 0.5, 0.7);
				prisma(text2.GLindex);                           //  Prisma E
			glPopMatrix();

			glTranslatef(0.375, 0.0, 0.0);         //Punto de rotación del dedo pulgar2 entre E y F
			glRotatef(movpulgar2, 0.0, 1.0, 0.0);  // Rotacion del pulgar de sobre eje Z de 0° a 90°
			glTranslated(0.375, 0.0, 0.0);          // Centro de Prisma F
			glScalef(0.75, 0.3, 0.25);
			glColor3f(0.2, 0.5, 0.7);
			prisma(text2.GLindex);                              // Prisma F
		glPopMatrix();

	// Dedo Medio
		glPushMatrix();                            // Regresamos al centro del prisma B
			glTranslatef(1.0, 0.35, 0.0);          // Punto de rotación del dedo pulgar entre B y G
			glRotatef(tijera2, 0.0, 1.0, 0.0);     // Giro de -10 sobre eje Y
			glRotatef(movmedio1, 0.0, 0.0, 1.0);   // Rotacion del pulgar de sobre eje Z de 0° a 90°
			glTranslated(0.5, 0.0, 0.0);           // Centro de Prisma G
			glPushMatrix();
				glScalef(1.0, 0.3, 0.25);
				glColor3f(1.0, 0.5, 0.0);
				prisma(text2.GLindex);                          //  Prisma G
			glPopMatrix();
			glTranslatef(0.5, 0.0, 0.0);          //Punto de rotación del dedo pulgar2 entre G y H
			glRotatef(movmedio2, 0.0, 0.0, 1.0);  // Rotacion del pulgar de sobre eje Z de 0° a 90°
			glTranslated(0.75, 0.0, 0.0);         // Centro de Prisma H
			glScalef(1.5, 0.3, 0.25);
			glColor3f(1.0, 0.5, 0.0);
			prisma(text2.GLindex);                             // Prisma H
		glPopMatrix();

	// Dedo Meñique
		glPushMatrix();                         // Regresamos al centro del prisma B
			glTranslatef(1.0, 0.35, -0.375);          // Punto de rotación del dedo pulgar entre B y I
			glRotatef(movmeñi1, 0.0, 0.0, 1.0);   // Rotacion del mañique de sobre eje Z de 0° a 90°
			glTranslated(0.5, 0.0, 0.0);           // Centro de Prisma I
			glPushMatrix();
				glScalef(1.0, 0.3, 0.25);
				glColor3f(0.5, 0.0, 0.0);
				prisma(text2.GLindex);                          //  Prisma I
			glPopMatrix();

			glTranslatef(0.5, 0.0, 0.0);          //Punto de rotación del dedo pulgar2 entre I y J
			glRotatef(movmeñi2, 0.0, 0.0, 1.0);  // Rotacion del meñique de sobre eje Z de 0° a 90°
			glTranslated(0.5, 0.0, 0.0);         // Centro de Prisma J
			glScalef(1.0, 0.3, 0.25);
			glColor3f(0.5, 0.0, 0.0);
			prisma(text2.GLindex);                             // Prisma J
		glPopMatrix();
}
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	objCamera.Position_Camera(0, 5.0f, 1, 0, 5.0f, 0, 0, 1, 0);////////////////////////////////////////////////////////////////////////////////7
	...
	KeyFrame[0].posY = 0;
	KeyFrame[0].posZ = -3.5;
	KeyFrame[0].movindi1 = -30;
	KeyFrame[0].movindi2 = -112;
	KeyFrame[0].movpulgar1 = 0;
	KeyFrame[0].movpulgar2 = 62;
	KeyFrame[0].movmedio1 = -30;
	KeyFrame[0].movmedio2 = -112;
	KeyFrame[0].movmeñi1 = -30;
	KeyFrame[0].movmeñi2 = -112;
	KeyFrame[0].tijera = 0;
	KeyFrame[0].tijera2 = 0;
	KeyFrame[0].señal = 2;
	...
	KeyFrame[1].posY = 0;
	KeyFrame[1].posZ = -3.5;
	KeyFrame[1].movindi1 = 0;
	KeyFrame[1].movindi2 = -6;
	KeyFrame[1].movpulgar1 = 0;
	KeyFrame[1].movpulgar2 = 0;
	KeyFrame[1].movmedio1 = 0;
	KeyFrame[1].movmedio2 = -6;
	KeyFrame[1].movmeñi1 = 0;
	KeyFrame[1].movmeñi2 = -6;
	KeyFrame[1].tijera = 0;
	KeyFrame[1].tijera2 = 0;
	KeyFrame[1].señal = -16;
	...
	KeyFrame[2].posY = 0;
	KeyFrame[2].posZ = -3.5;
	KeyFrame[2].movindi1 = 0;
	KeyFrame[2].movindi2 = 0;
	KeyFrame[2].movpulgar1 = -2;
	KeyFrame[2].movpulgar2 = 92;
	KeyFrame[2].movmedio1 = 0;
	KeyFrame[2].movmedio2 = 0;
	KeyFrame[2].movmeñi1 = -30;
	KeyFrame[2].movmeñi2 = -112;
	KeyFrame[2].tijera = -5;
	KeyFrame[2].tijera2 = 5;
	KeyFrame[2].señal = 2;
	...
	KeyFrame[3].posY = 0;
	KeyFrame[3].posZ = -3.5;
	KeyFrame[3].movindi1 = -30;
	KeyFrame[3].movindi2 = -112;
	KeyFrame[3].movpulgar1 = 0;
	KeyFrame[3].movpulgar2 = 0;
	KeyFrame[3].movmedio1 = -30;
	KeyFrame[3].movmedio2 = -112;
	KeyFrame[3].movmeñi1 = -30;
	KeyFrame[3].movmeñi2 = -112;
	KeyFrame[3].tijera = 0;
	KeyFrame[3].tijera2 = 0;
	KeyFrame[3].señal = -92;
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			movbrazoderecho += KeyFrame[playIndex].movbrazoderechoInc;
			movpiernader += KeyFrame[playIndex].movpiernaderInc;
			
			movindi1 += KeyFrame[playIndex].movindi1Inc;
			movindi2 += KeyFrame[playIndex].movindi2Inc;

			movpulgar1 += KeyFrame[playIndex].movpulgar1Inc;
			movpulgar2 += KeyFrame[playIndex].movpulgar2Inc;

			movmedio1 += KeyFrame[playIndex].movmedio1Inc;
			movmedio2 += KeyFrame[playIndex].movmedio2Inc;

			movmeñi1 += KeyFrame[playIndex].movmeñi1Inc;
			movmeñi2 += KeyFrame[playIndex].movmeñi2Inc;

			tijera += KeyFrame[playIndex].tijeraInc;
			tijera2 += KeyFrame[playIndex].tijera2Inc;

			señal += KeyFrame[playIndex].señalInc;
